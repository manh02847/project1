...
{
    [Route("/NnmAccount", Name = "account")]
    public class NnmAccountController : Controller
    {
        private readonly List<NnmAccount> nnmAccounts = new()
        {
            new NnmAccount
            {
                Id = 1,
        ...
        {
            ViewBag.NnmAccounts = nnmAccounts;
            return View();
        }

        [Route("ho-so-cua-toi", Name = "nnmprofile")]
        public IActionResult NnmProfile(int? id)
        {
            NnmAccount nnmAccount = new NnmAccount
            {
                Id = 5,
                Name = "Hoàng Quốc Dũng",
                Email = "dung.hoang@example.com",
            ...
            if (id != null)
                nnmAccount = nnmAccounts.FirstOrDefault(x => x.Id == id) ?? nnmAccount;

            ViewBag.NnmAccount = nnmAccount;
            return View();
        }
    }
}
